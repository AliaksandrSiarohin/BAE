...
def get_image_memorability(image):
    if not hasattr(get_image_memorability, 'mem_net'):
        get_image_memorability.mem_net = mem_model()
...
def mem_score(stylized_image):
    stylized_score = get_image_memorability(stylized_image)
...
def get_score(stylized_image, z_style, weight_image=20, weight_prior=1, image_score_fun='blue'):
    ...
    isf = weight_image * score_funs[image_score_fun](stylized_image)
